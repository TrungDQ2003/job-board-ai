...
async function SuspendedComponent({ searchParams, params }: Props) {
  ...
  if (jobListings.length === 0) {
    return (
      <div className="text-muted-foreground p-4">{t("jobListings.noJobs")}</div>
    )
  }
...
async function DaysSincePosting({ postedAt }: { postedAt: Date }) {
  ...
  if (daysSincePosted === 0) {
    return <Badge>{language === "vi" ? "Mới" : "New"}</Badge>
  }

  return new Intl.RelativeTimeFormat(language === "vi" ? "vi-VN" : "en-US", {
    style: "narrow",
    numeric: "always",
