  ...
  return (
    <div className="space-y-6 @container">
      <div className="space-y-4">
        <div className="flex gap-4 items-start">
          ...
          <div className="ml-auto flex items-center gap-4">
            ...
            <Button size="icon" variant="outline" asChild>
              ...
              >
                <span className="sr-only">{t("jobDetails.close")}</span>
                <XIcon />
              </Link>
            </Button>
          </div>
        ...
        <Suspense fallback={<Button disabled>{t("jobDetails.apply")}</Button>}>
...
async function ApplyButton({ jobListingId }: { jobListingId: string }) {
    ...
    return (
      <Popover>
        <PopoverTrigger asChild>
          <Button>{t("jobDetails.apply")}</Button>
        </PopoverTrigger>
        <PopoverContent className="flex flex-col gap-2">
          {t("jobDetails.needAccount")}
          <SignUpButton />
        </PopoverContent>
      </Popover>
    )
  ...
  if (application != null) {
    const formatter = new Intl.RelativeTimeFormat(language === "vi" ? "vi-VN" : "en-US", {
      style: "short",
      numeric: "always",
    })
    ...
    return (
      <div className="text-muted-foreground text-sm">
        ...
        {difference === 0 ? t("jobDetails.today") : formatter.format(difference, "days")}
      </div>
    )
  }
  ...
  if (userResume == null) {
    return (
      <Popover>
        ...
        <PopoverContent className="flex flex-col gap-2">
          {t("jobDetails.needResume")}
          <Button asChild>
            <Link href="/user-settings/resume">{t("jobDetails.uploadResume")}</Link>
          </Button>
        </PopoverContent>
      </Popover>
    )
  ...
  return (
    <Dialog>
      ...
      <DialogContent className="md:max-w-3xl max-h-[calc(100%-2rem)] overflow-hidden flex flex-col">
        <DialogHeader>
          <DialogTitle>{t("jobDetails.application")}</DialogTitle>
          <DialogDescription>
            {t("jobDetails.cannotBeUndone")}
          </DialogDescription>
        </DialogHeader>
