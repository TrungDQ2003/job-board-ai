...
const statusKeys: Record<JobListingStatus, string> = {
  published: "employer.statusActive",
  draft: "employer.statusDraft",
  delisted: "employer.statusDelisted",
}
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto p-4 @container">
      <div className="flex items-center justify-between gap-4 @max-4xl:flex-col @max-4xl:items-start">
        <div>
          ...
          <div className="flex flex-wrap gap-2 mt-2">
            <Badge>{t(statusKeys[jobListing.status])}</Badge>
            <JobListingBadges jobListing={jobListing} t={t} />
          </div>
        </div>
        <div className="flex items-center gap-2 empty:-mt-4">
          ...
          <StatusUpdateButton status={jobListing.status} id={jobListing.id} t={t} />
      ...
      <MarkdownPartial
        ...
        dialogTitle={t("jobListings.description")}
      />

      <Separator />

      <div className="space-y-6">
        <h2 className="text-xl font-semibold">{t("employer.applications")}</h2>
...
function StatusUpdateButton({
  ...
  const button = (
    <ActionButton
      ...
      areYouSureDescription={t("employer.areYouSurePublish")}
    >
      {statusToggleButtonText(status, t)}
    </ActionButton>
  )
    ...
    >
      {getNextJobListingStatus(status) === "published" ? (
        <AsyncIf
          ...
          otherwise={
            <UpgradePopover
              buttonText={statusToggleButtonText(status, t)}
              popoverText={t("employer.maxPublishedError")}
              t={t}
            />
          }
        >
...
function FeaturedToggleButton({
  ...
  const button = (
    <ActionButton
      ...
      variant="outline"
    >
      {featuredToggleButtonText(isFeatured, t)}
    </ActionButton>
  )
    ...
    >
      {isFeatured ? (
        ...
        <AsyncIf
          ...
          otherwise={
            <UpgradePopover
              buttonText={featuredToggleButtonText(isFeatured, t)}
              popoverText={t("employer.maxFeaturedError")}
              t={t}
            />
          }
        >
  ...
  return (
    <Popover>
      ...
      <PopoverContent className="flex flex-col gap-2">
        {popoverText}
        <Button asChild>
          <Link href="/employer/pricing">{t("employer.upgradePlan")}</Link>
        </Button>
      </PopoverContent>
    </Popover>
  )
...
function statusToggleButtonText(status: JobListingStatus, t: any) {
  switch (status) {
    case "delisted":
    case "draft":
      return (
...
function featuredToggleButtonText(isFeatured: boolean, t: any) {
  if (isFeatured) {
    return (
      <>
        <StarOffIcon className="size-4" />
        {t("employer.unfeature")}
      </>
    )
  }
